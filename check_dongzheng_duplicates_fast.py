#!/usr/bin/env python3
"""
check_dongzheng_duplicates_fast.py - 快速检查dongzheng/full_tick数据中各交易所的重复记录情况
...
def check_duplicates_by_exchange(sample_size=500, full_check=False):
    """检查各交易所的重复数据"""
    ...
    # 统计结果
    stats = defaultdict(lambda: {'total': 0, 'duplicates': 0, 'contracts': [], 'files_checked': 0})
    ...
    # 采样或全量检查
    if not full_check and total_files > sample_size:
        print(f"📊 采用抽样模式，从 {total_files} 个合约中随机抽取 {sample_size} 个检查...")
        files = random.sample(files, sample_size)
    else:
        print(f"📊 全量检查模式，处理 {total_files} 个合约...")
    ...
    for file in files:
        ...
        try:
            ...
            stats[exchange]['files_checked'] += 1
            ...
            processed += 1
            if processed % 50 == 0:
                print(f"进度: {processed}/{len(files)} ({processed/len(files)*100:.1f}%)")

        except Exception as e:
            print(f"⚠️ 处理 {contract} 时出错: {e}")
            continue

    print(f"\n✅ 处理完成，共检查 {processed} 个合约")
    return stats

def print_report(stats, is_sample=False):
    """打印重复数据报告"""
    print("\n" + "=" * 80)
    if is_sample:
        print("📊 各交易所重复数据统计报告（抽样结果）")
    else:
        print("📊 各交易所重复数据统计报告（全量数据）")
    print("=" * 80)
    ...
    # 打印总表
    print(f"\n{'排名':<6} {'交易所':<8} {'总记录数':<15} {'重复记录':<15} {'重复率':<10} {'检查文件数':<12} {'特征':<10}")
    print("-" * 95)
    ...
    for i, summary in enumerate(exchange_summary, 1):
        ...
        print(f"{emoji:<6} {summary['exchange']:<8} "
              ...
              f"{rate:>9.2f}% {summary['files_checked']:>11,} {feature:<10}")
...
def main():
    ...
    if full_check:
        print("⚠️ 使用全量检查模式，这可能需要较长时间...")
    else:
        print(f"💡 使用抽样模式（{sample_size}个合约），如需全量检查请使用 --full 参数")

    print()
    stats = check_duplicates_by_exchange(sample_size=sample_size, full_check=full_check)

    if stats:
        print_report(stats, is_sample=not full_check)
