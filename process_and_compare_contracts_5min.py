#!/usr/bin/env python3
# process_and_compare_contracts_5min.py
# 从1min数据生成5min数据，并对比on_batch和on_tick的5min结果
#
# 前提: 1min数据已经生成（full_bar_1min/ 和 full_bar_1min_on_tick/）
#
# 用法:
#   python3 process_and_compare_contracts_5min.py                    # 默认处理2501合约
#   python3 process_and_compare_contracts_5min.py --patterns 2505    # 只处理2505合约
#   python3 process_and_compare_contracts_5min.py --patterns 2501 2502  # 处理2501和2502合约
...
def find_contracts_in_1min(patterns):
    """从full_bar_1min目录扫描符合条件的合约"""
    bar_1min_dir = Path("/data/future_data/dongzheng_data/full_bar_1min")

    if not bar_1min_dir.exists():
        print(f"❌ 1min数据目录不存在: {bar_1min_dir}")
        return []

    # 扫描所有1min bar文件
    bar_files = list(bar_1min_dir.glob("*_1min.parquet"))

    contracts = []
    for f in bar_files:
        # 移除_1min后缀获取合约名
        contract = f.stem.replace("_1min", "")
...
def process_contract(contract, mode):
    ...
    """
    if mode == 'batch':
        cmd = f"./target/release/parquet_processor_on_batch dongzheng_data 5min {contract}"
    elif mode == 'tick':
        cmd = f"./target/release/parquet_processor_on_tick dongzheng_data 5min {contract}"
    else:
        print(f"❌ 未知模式: {mode}")
        return False
...
def quick_compare_contract(contract):
    ...
    batch_file = f"/data/future_data/dongzheng_data/full_bar_5min/{contract}_5min.parquet"
    tick_file = f"/data/future_data/dongzheng_data/full_bar_5min_on_tick/{contract}_5min.parquet"
...
def main():
    parser = argparse.ArgumentParser(description='处理并对比on_batch和on_tick的5min结果')
    ...
    print("=" * 120)
    print("处理并对比合约 - 5min bars - on_batch vs on_tick")
    print("=" * 120)
    print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"合约模式: {', '.join(args.patterns)}")
    print("前提: 1min数据已存在（full_bar_1min/ 和 full_bar_1min_on_tick/）")

    # 1. 扫描合约
    print("\n" + "=" * 120)
    print("【步骤1】扫描1min数据中的合约")
    print("=" * 120)

    contracts = find_contracts_in_1min(args.patterns)
    ...
    if not args.skip_processing:
        # 2. 生成5min数据（on_batch）
        print("\n" + "=" * 120)
        print("【步骤2】生成5min数据 - on_batch (1min -> 5min)")
        print("=" * 120)
        ...
        # 3. 生成5min数据（on_tick）
        print("\n" + "=" * 120)
        print("【步骤3】生成5min数据 - on_tick (1min -> 5min)")
        print("=" * 120)
